...
Non-rotatable errors (400, 404) are deliberately _not_ retried against other keys, since the same malformed request would simply fail identically against any key retrying would waste latency and API calls without changing the outcome.

**Consequences:** A key can be selected again before an operator has confirmed the underlying issue is actually resolved (since blacklist expiry is time-based, not health-check-based). This is accepted as a reasonable default; if the issue persists, the key will simply fail and be blacklisted again provided there are additional keys in the pool.
