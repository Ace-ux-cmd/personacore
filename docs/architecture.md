...
Both `ArrayStore` and `MongoStore` implement this identically from the caller's perspective, returning `{ role, text, createdAt }` records. The two backends differ only in *where* the enforcement and trimming happen:
